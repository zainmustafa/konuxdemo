...
describe("utils/api", () => {
    ...
    it("should execute a POST successfully", done => {
        xhrMock.post("/token", (req, res) => res.status(201).body("ok"));

        request({ url: "/token", method: "POST", payload: { a: 1 } }).then(
            data => {
    ...
    it("should fail to a PUT without payload", () => {
        expect(() => request({ url: "/token", method: "PUT" })).toThrow(
    ...
    it("should reject for a server error with no response", done => {
        xhrMock.get("token", (req, res) => res.status(500));
